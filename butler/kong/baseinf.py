...
from ..kong import _KONGADM_URL, _KONGADM_APIKEY, _KONGADM_BASICAUTH, \
    _USE_SESSION
...
client = Client(base_url=_KONGADM_URL, apikey=_KONGADM_APIKEY,
                basic_auth=_KONGADM_BASICAUTH, use_session=_USE_SESSION)
...
class AclPluginInf(PluginInf):
    ...
    def list(self, api_id=None, **kwargs):
        if api_id is not None:
    ...
    def delete(self, api_id, **kwargs):
        num, acl_plugins = self.list(api_id)
        if num:
            for acl_plugin in acl_plugins:
                super(AclPluginInf, self).delete(acl_plugin['id'], api_id)
...
class JwtPluginInf(PluginInf):
    ...
    def __init__(self, api_id, http_client=client):
        super(JwtPluginInf, self).__init__(client)
        self._plugin_name = 'jwt'
        self._api_id = api_id

    def add(self, api_id, claims_to_verify=['exp', 'nbf'],
            run_on_preflight=True, **kwargs):
        num, jwt_plugins = self.list()
        if num:
            return jwt_plugins[0]
        config_params = dict()
    ...
    def update(self, plugin_id, claims_to_verify=None, run_on_preflight=None,
        ...
        if run_on_preflight is not None:
            config_params['run_on_preflight'] = run_on_preflight
        return super(JwtPluginInf, self).update(
            plugin_id=plugin_id,
    ...
    def list(self, **kwargs):
        return super(JwtPluginInf, self). \
            list(plugin_name=self._plugin_name, api_id=self._api_id)

    def delete(self, plugin_id, **kwargs):
        return super(JwtPluginInf, self). \
            delete(plugin_id=plugin_id, api_name_or_id=self._api_id)

    def remove(self, api_id):
        total, infos = self.list()
        if total:
            for info in infos:
                self.delete(info['id'])
...
class JwtCredInf(object):
    ...
    def __init__(self, username_or_id, allow_duplicated=False, http_client=client):
        """
    ...
    def token_gen(key, secret, token_algorithm=TOKEN_ALGORITHM,
        ...
        claims['iss'] = key
        claims['nbf'] = datetime.datetime.utcnow() - datetime.timedelta(seconds=120)

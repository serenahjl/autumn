...
def hash_pass(password):
    ...
    salted_password = hashlib.md5(secreted_password).hexdigest() + \
        app.config['PSW_SALT']
...
def logmsg_req(msg, url, method, status_code, resp_data):
    return logmsg(
        '%s(%s[%s]%d:%s)' % (msg, url, method, status_code, resp_data))
...
def to_dict(inst, cls=None, except_clm_list=None, target_clm_list=None,
            ext_dict=None):
    if isinstance(inst, list):
        ...
        if cls is None:
            cls = inst.__class__
        return _to_dict(
            inst, cls, except_clm_list=except_clm_list,
            target_clm_list=target_clm_list,
            ext_dict=ext_dict)


def _to_dict(inst, cls, except_clm_list=None, target_clm_list=None,
             ext_dict=None):
    """
